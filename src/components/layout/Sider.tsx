...
const Sider: FC = () => {
    ...
    const { toggleBold, toggleItalic, toggleUnderline, toggleCode, activeFormats } =
        ...
        <div className={"sider"}>
            <div className={"earth-button-wrapper"}>
                <div className={`info ${gaiaBarOpen && "hide"}`}>
                    <Tooltip
                        title={
                            "Environmental savings per (calculated per token) by using our models compared to existing SOTA models. For each token you input or output, we calculate tree mass, volume of water, mass of carbon dioxide (CO2), joules of energy, and land mass conserved using the bilateral cortex model."
                        }
                        placement={"right"}
                        overlayInnerStyle={{
                ...
                {gaiaBarOpen && (
                    <div className={"gaia-bar-container"}>
            ...
            <div className={"sider-control-buttons"}>
                {buttons.map(({ icon, action, active }) => (
                    <ButtonWithIcon key={icon} icon={icon} active={active} onClick={action} />
                ))}
            </div>

            <ButtonWithIcon active onClick={deleteChat} icon={"trashCan"} />
        </div>
    );
};
