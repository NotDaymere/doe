...
interface ChatMessageProps {
  ...
  isUser?: boolean
  isCode?: boolean
  onEditMessage: React.Dispatch<React.SetStateAction<{
    id: number | null;
  }>>
}
  ...
  return (
    <Flex vertical>
      <div className={`message-container ${isUser ? 'user' : 'bot'}`}>
        ...
        {isUser && <div className={'edit-button'} onClick={handleSetEditMessage}>
          <SvgIcon type={'pencil'} />
        </div>}
          ...
          <div className={"message-content"}>
              ...
              <MathJax>
                {parsedContent.map((part, index) => {
                  if (part.type === "text" && !isUser) {
                    return <div dangerouslySetInnerHTML={{ __html: parseTextFormatting(part.content) }} />
            ...
            {!isUser && (
              <Flex justify={"space-between"} className={"message-actions"}>
                <Button
                  icon={<SvgIcon style={{ width: "15px", height: "15px" }} type={"seeAllStepsIcon"} />}
                ...
                <Flex gap={10}>
                  ...
                  <Button onClick={downloadPDF} icon={<SvgIcon style={{ width: "15px", height: "15px" }} type={"downloadAnswerIcon"} />} />
                  <Button icon={<SvgIcon style={{ width: "15px", height: "15px" }} type={"copyAnswerIcon"} />} onClick={handleCopy} />
                </Flex>
              </Flex>
            )}
          </div>
      ...
      {files && <Flex gap={4} style={{ flexWrap: 'nowrap', overflowX: 'auto' }}>
