...
const parseLayout = (
    chartElement: Element
): { hAxis?: { title: string }; vAxis?: { title: string } } => {
...
const parseStyle = (chartElement: Element): { [key: string]: any } => {
    ...
    const donutOptions = innerRadius
        ? {
              pieHole: innerRadius.includes("%") ? parseFloat(innerRadius) / 100 : innerRadius,
          }
        : {};

    return {
        bar: { groupWidth: barWidth },
        colors: colors.length ? colors : colorPalette,
        ...candlestickOptions,
        ...donutOptions,
    };
};
...
const parseGanttChartData = (
    dataElement: Element
): [Array<{ type: string; label: string }>, ...Array<Array<string | number | Date>>] => {
    ...
    tasks.forEach((task: Element, index: number) => {
        ...
        rows.push([`Task-${index + 1}`, taskName, startDate, endDate, duration, 100, ""]);
    });
...
const parseBulletChartData = (dataElement: Element): Array<Record<string, any>> => {
    const performance = JSON.parse(
        dataElement.getElementsByTagName("performance")[0]?.textContent || "[]"
    );
...
const parseDensityData = (dataElement: Element): Array<{ x: number; y: number }> => {
    ...
    const densityValuesContent =
        dataElement.getElementsByTagName("densityvalues")[0]?.textContent || "[]";
...
const parseVennData = (
    dataElement: Element
): Array<{ sets: string[]; size: number; label?: string }> => {
                                    ...
                                    children={[
                                        {
                                            ...
                                            style: {
                                                fill: (d: { percentage: number }) =>
                                                    d.percentage < 0.1 ? "#FF4848" : "#FE9202",
                                            },
                                            axis: {
                                                x: { title: null },
