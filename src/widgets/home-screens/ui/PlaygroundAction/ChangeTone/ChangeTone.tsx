...
function ChangeTone({ editor }: Props) {
    const [changeToneOption, setToneOption] = useState<string>("");
        ...
        <React.Fragment>
            <div className={"writing-level-content"}>
                <p className={"text-columns-button-p"}>
                    Change<span className={"text-columns-button-span"}>Tone</span>to
                </p>
                <div className={"writing-level-option"}>
                    <div className={"writing-level-option-text"}>{changeToneOption}</div>
                    <button className={"writing-level-option-button"} onClick={() => setOpenMenu(!openMenu)}>
                        <PenIcon className={"pen-icon-writing-level"} />
                    </button>
                    {openMenu && <Menu setOpenMenu={setOpenMenu} setWritingLevelOption={setToneOption} />}
                </div>
